...
| 시작 화면 | 채널 등록 | 대시보드 |
|---|---|---|
| ![시작 화면](./docs/screenshots/home.png) | ![채널 등록](./docs/screenshots/channels.png) | ![대시보드](./docs/screenshots/dashboard.png) |
...
```
실행하면 터미널에 **토큰이 포함된 주소**가 뜨는데, 그 주소로 접속하세요 (예: `http://localhost:5000/?token=xxxxxxxx`). 한 번 접속한 브라우저는 이후 토큰 없이 `http://localhost:5000`으로 바로 들어가도 돼요.

> ⚠️ 이 웹 UI는 폰에서도 볼 수 있도록 같은 와이파이의 다른 기기에서도 접속 가능하게 열려 있어요. 그래서 토큰을 모르면 아무도 못 들어오게 막아놨어요 — 토큰이 없는 주소(`http://localhost:5000`)로 처음 접속하면 403 에러가 뜨는 게 정상이에요. 폰에서 접속하려면 `localhost` 대신 이 컴퓨터의 IP 주소를 쓰고, `?token=...`을 똑같이 붙이면 돼요. (토큰은 `config/webui.token`에 저장되고, 저장소에는 올라가지 않아요.)
